...
function createPathContext (lineCurveFactory, polygonCurveFactory) {
  ...
  PathContext.prototype = {
    ...
    lineStart: function () {
      this._point = 0;
      if (this._line === 0) {
        this._polygonCurve && this._polygonCurve.lineStart();
      } else {
        this._lineCurve && this._lineCurve.lineStart();
      }
    },
    lineEnd: function () {
      if (this._line === 0) {
        this._polygonCurve && this._polygonCurve.lineEnd();
      } else {
        this._lineCurve && this._lineCurve.lineEnd();
      }
      this._point = NaN;
    },
    point: function (x, y) {
      if (this._point === 0) {
        if (this._line === 0) {
          this._polygonCurve && this._polygonCurve.point(x, y);
        } else {
          this._lineCurve && this._lineCurve.point(x, y);
        }
      } else {
